...
export default function EnglishTranslationChapter21() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter22 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter22')
  }

 const loop21=[119,120,121]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 21. The Sitting Of Rasoolullah</h2> </center>

    {
      loop21.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter21' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
     {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter21' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter22} >View Next Chapter{'>>'}</button>
           </div>
