...
export default function EnglishTranslationChapter35() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter36 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter36')
  }

 const loop35=[213,214,215,216,217,218,219,220]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 35. The Laughing Of Rasoolullah</h2> </center>

    {
      loop35.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter35' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter35' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter36} >View Next Chapter{'>>'}</button>
           </div>
