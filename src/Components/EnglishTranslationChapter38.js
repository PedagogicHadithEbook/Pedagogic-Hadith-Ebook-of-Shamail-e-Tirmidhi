...
export default function EnglishTranslationChapter38() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter39 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter39')
  }

 const loop38=[237,238,239,240,241,242]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 38. The Sleeping Of Rasoolullah</h2></center>

    {
      loop38.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter38' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter38' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter39} >View Next Chapter{'>>'}</button>
           </div>
