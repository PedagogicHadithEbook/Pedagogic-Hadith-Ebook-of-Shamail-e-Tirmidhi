...
export default function EnglishTranslationChapter45() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter46 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter46')
  }
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 45. Narrations Of The Bed Of Sayyidina Rasoolullah</h2></center>

    {
      loop45.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter45' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter45' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter46} >View Next Chapter{'>>'}</button>
           </div>
