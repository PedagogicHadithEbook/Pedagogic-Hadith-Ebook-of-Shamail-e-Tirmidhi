...
export default function EnglishTranslationChapter41() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter42 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter42')
  }
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 41. Sayyidina Rasoolullah Performing Nawaafil At Home</h2></center>

    {
      loop41.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter41' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter41' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter42} >View Next Chapter{'>>'}</button>
           </div>
