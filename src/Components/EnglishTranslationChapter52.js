...
export default function EnglishTranslationChapter52() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter53 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter53')
  }

 const loop52=[354,355,356,357,358,359]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 52. The Noble Age Of Sayyidina Rasoolullah</h2></center>

    {
      loop52.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter52' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter52' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter53} >View Next Chapter{'>>'}</button>
           </div>
