...
export default function EnglishTranslationChapter12() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter13 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter13')
  }

 const loop12=[81,82,83,84,85,86,87,88]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 12. The Mubarak Ring Of Rasoolullah</h2> </center>

    {
      loop12.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter12' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter12' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter13} >View Next Chapter{'>>'}</button>
           </div>
