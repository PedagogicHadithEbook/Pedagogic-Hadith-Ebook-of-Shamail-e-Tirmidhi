...
export default function EnglishTranslationChapter24() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter25 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter25')
  }
 
 const loop24=[128,129,130,131,132]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 24. Description Of The Eating Of Rasoolullah</h2> </center>

    {
      loop24.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter24' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter24' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter25} >View Next Chapter{'>>'}</button>
           </div>
