...
export default function EnglishTranslationChapter23() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter24 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter24')
  }

 const loop23=[126,127]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 23. Rasoolullah Leaning On Something Other Than a Pillow</h2> </center>

    {
      loop23.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter23' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter23' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter24} >View Next Chapter{'>>'}</button>
           </div>
