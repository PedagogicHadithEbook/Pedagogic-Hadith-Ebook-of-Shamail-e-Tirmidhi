...
export default function UrduTranslationChapter25() {
  
  const navigate = useNavigate();
  
  const UrduTranslationChapter26 = () => {
      navigate('/UrduTranslationChapters/UrduTranslationChapter26')
  }

 const loop25=[133,134,135,136,137,138,139,140]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 25. The Bread Of Rasoolullah</h2> </center>
    {/* hadith container */}
     {
      loop25.map((loop,index) => (  
    ...
    <RWebShare
      ...
      {/* eslint-disable-next-line */}
      <h6><HashLink to={"/EnglishTranslationChapters/EnglishTranslationChapter25"+"/#Hadith"+(loop+1)} style={{ color:"midnightblue"}}>View English Translation</HashLink></h6>
       {/* eslint-disable-next-line */}
      <h6><HashLink to={"/WordToWordTranslationChapters/WordToWordTranslationChapter25"+"/#Hadith"+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {UrduTranslationChapter26} >View Next Chapter{'>>'}</button>
           </div>
   </>
  )
}
