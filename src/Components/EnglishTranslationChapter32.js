...
export default function EnglishTranslationChapter32() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter33 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter33')
  }

 const loop32=[194,195,196,197,198,199,200,201,202,203]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 32. Hadith Describing The Manner Rasoolullah Drank</h2> </center>

    {
      loop32.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter32' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
     {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter32' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter33} >View Next Chapter{'>>'}</button>
           </div>
