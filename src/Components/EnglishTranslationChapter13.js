...
export default function EnglishTranslationChapter13() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter14 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter14')
  }

 const loop13=[89,90,91,92,93,94,95,96,97]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 13. Stating That Rasoolullah Wore The Ring On His Right Hand</h2> </center>

    {
      loop13.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter13' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
     {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter13' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter14} >View Next Chapter{'>>'}</button>
           </div>
