...
export default function UrduTranslationChapter43() {
  
  const navigate = useNavigate();
  
  const UrduTranslationChapter44 = () => {
      navigate('/UrduTranslationChapters/UrduTranslationChapter44')
  }
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <RWebShare
      ...
      {/* eslint-disable-next-line */}
      <h6><HashLink to={"/EnglishTranslationChapters/EnglishTranslationChapter43"+"/#Hadith"+(loop+1)} style={{ color:"midnightblue"}}>View English Translation</HashLink></h6>
       {/* eslint-disable-next-line */}
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {UrduTranslationChapter44} >View Next Chapter{'>>'}</button>
           </div>
   </>
  )
}
