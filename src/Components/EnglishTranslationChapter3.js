...
export default function EnglishTranslationChapter3() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter4 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter4')
  }

 const loop3=[22,23,24,25,26,27,28,29]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 3. The Mubarak Hair Of Rasoolullah</h2> </center>

    {
      loop3.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter3' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter3' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter4} >View Next Chapter{'>>'}</button>
           </div>
