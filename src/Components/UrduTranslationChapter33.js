...
export default function UrduTranslationChapter33() {
  
  const navigate = useNavigate();
  
  const UrduTranslationChapter34 = () => {
      navigate('/UrduTranslationChapters/UrduTranslationChapter34')
  }
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <RWebShare
      ...
      {/* eslint-disable-next-line */}
      <h6><HashLink to={"/EnglishTranslationChapters/EnglishTranslationChapter33"+"/#Hadith"+(loop+1)} style={{ color:"midnightblue"}}>View English Translation</HashLink></h6>
       {/* eslint-disable-next-line */}
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {UrduTranslationChapter34} >View Next Chapter{'>>'}</button>
           </div>
   </>
  )
}
