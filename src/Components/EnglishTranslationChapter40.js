...
export default function EnglishTranslationChapter40() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter41 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter41')
  }

 const loop40=[267,268,269,270,271,272,273,274]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 40. Salaatut Duha (Chaast Prayers)</h2></center>

    {
      loop40.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter40' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter40' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter41} >View Next Chapter{'>>'}</button>
           </div>
