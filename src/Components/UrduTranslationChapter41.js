...
export default function UrduTranslationChapter41() {
  
  const navigate = useNavigate();
  
  const UrduTranslationChapter42 = () => {
      navigate('/UrduTranslationChapters/UrduTranslationChapter42')
  }
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <RWebShare
      ...
      {/* eslint-disable-next-line */}
      <h6><HashLink to={"/EnglishTranslationChapters/EnglishTranslationChapter41"+"/#Hadith"+(loop+1)} style={{ color:"midnightblue"}}>View English Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {UrduTranslationChapter42} >View Next Chapter{'>>'}</button>
           </div>
   </>
  )
}
