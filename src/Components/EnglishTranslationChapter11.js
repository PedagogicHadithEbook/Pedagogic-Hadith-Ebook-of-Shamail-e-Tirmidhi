...
export default function EnglishTranslationChapter11() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter12 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter12')
  }

 const loop11=[70,71,72,73,74,75,76,77,78,79,80]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 11. The Shoes Of Rasoolullah</h2> </center>

    {
      loop11.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter11' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter11' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
     </div>
           </div>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter12} >View Next Chapter{'>>'}</button>
           </div>
