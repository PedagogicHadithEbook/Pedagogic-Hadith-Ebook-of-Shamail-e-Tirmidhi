...
export default function UrduTranslationChapter19() {
  
  const navigate = useNavigate();
  
  const UrduTranslationChapter20 = () => {
      navigate('/UrduTranslationChapters/UrduTranslationChapter20')
  }
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <RWebShare
      ...
      {/* eslint-disable-next-line */}
      <h6><HashLink to={"/EnglishTranslationChapters/EnglishTranslationChapter19"+"/#Hadith"+(loop+1)} style={{ color:"midnightblue"}}>View English Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {UrduTranslationChapter20} >View Next Chapter{'>>'}</button>
           </div>
   </>
  )
}
