...
export default function EnglishTranslationChapter14() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter15 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter15')
  }

 const loop14=[98,99,100,101]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 14. The Sword Of Rasoolullah</h2> </center>

    {
      loop14.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter14' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter14' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter15} >View Next Chapter{'>>'}</button>
           </div>
