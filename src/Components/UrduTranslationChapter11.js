...
export default function UrduTranslationChapter11() {
  
  const navigate = useNavigate();
  
  const UrduTranslationChapter12 = () => {
      navigate('/UrduTranslationChapters/UrduTranslationChapter12')
  }
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <RWebShare
      ...
      {/* eslint-disable-next-line */}
      <h6><HashLink to={"/EnglishTranslationChapters/EnglishTranslationChapter11"+"/#Hadith"+(loop+1)} style={{ color:"midnightblue"}}>View English Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {UrduTranslationChapter12} >View Next Chapter{'>>'}</button>
           </div>
   </>
  )
}
