...
export default function EnglishTranslationChapter47() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter48 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter48')
  }

 const loop47=[320,321,322,323,324,325,326,327,328,329,330,331,332,333,334]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 47. Noble Character And Habits Of Sayyidina Rasoolullah</h2></center>

    {
      loop47.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter47' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter47' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter48} >View Next Chapter{'>>'}</button>
           </div>
