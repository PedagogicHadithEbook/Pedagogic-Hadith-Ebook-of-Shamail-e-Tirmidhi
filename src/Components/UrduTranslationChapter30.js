...
export default function UrduTranslationChapter30() {
  
  const navigate = useNavigate();
  
  const UrduTranslationChapter31 = () => {
      navigate('/UrduTranslationChapters/UrduTranslationChapter31')
  }
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <RWebShare
      ...
      {/* eslint-disable-next-line */}
      <h6><HashLink to={"/EnglishTranslationChapters/EnglishTranslationChapter30"+"/#Hadith"+(loop+1)} style={{ color:"midnightblue"}}>View English Translation</HashLink></h6>
       {/* eslint-disable-next-line */}
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {UrduTranslationChapter31} >View Next Chapter{'>>'}</button>
           </div>
   </>
  )
}
