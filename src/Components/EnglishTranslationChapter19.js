...
export default function EnglishTranslationChapter19() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter20 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter20')
  }

 const loop19=[115,116,117]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 19. The Walking Of Rasoolullah</h2> </center>

    {
      loop19.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter19' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter19' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter20} >View Next Chapter{'>>'}</button>
           </div>
