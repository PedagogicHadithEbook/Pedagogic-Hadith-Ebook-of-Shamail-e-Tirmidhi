...
export default function UrduTranslationChapter27() {
  
  const navigate = useNavigate();
  
  const UrduTranslationChapter28 = () => {
      navigate('/UrduTranslationChapters/UrduTranslationChapter28')
  }


 const loop27=[174,175,176]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <RWebShare
      ...
      {/* eslint-disable-next-line */}
      <h6><HashLink to={"/EnglishTranslationChapters/EnglishTranslationChapter27"+"/#Hadith"+(loop+1)} style={{ color:"midnightblue"}}>View English Translation</HashLink></h6>
       {/* eslint-disable-next-line */}
      <h6><HashLink to={"/WordToWordTranslationChapters/WordToWordTranslationChapter27"+"/#Hadith"+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {UrduTranslationChapter28} >View Next Chapter{'>>'}</button>
           </div>
   </>
  )
}
