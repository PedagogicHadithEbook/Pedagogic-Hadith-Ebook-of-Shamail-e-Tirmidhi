...
export default function EnglishTranslationChapter42() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter43 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter43')
  }

 const loop42=[276,277,278,279,280,281,282,283,284,285,286,287,288,289,290]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 42. The Fasting Of Sayyidina Rasoolullah</h2></center>

    {
      loop42.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter42' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter42' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter43} >View Next Chapter{'>>'}</button>
           </div>
