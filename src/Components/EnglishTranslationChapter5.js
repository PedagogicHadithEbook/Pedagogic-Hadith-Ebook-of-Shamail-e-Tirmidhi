...
export default function EnglishTranslationChapter5() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter6 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter6')
  }

  
 const loop5=[35,36,37,38,39,40,41,42]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 5. Appearing Of The White Hair Of Rasoolullah</h2> </center>

    {
      loop5.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter5' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter5' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter6} >View Next Chapter{'>>'}</button>
           </div>
