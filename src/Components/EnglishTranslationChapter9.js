...
export default function EnglishTranslationChapter9() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter10 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter10')
  }
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 9. The Standard of Living of Rasoolullah</h2> </center>

    {
      loop9.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter9' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter9' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter10} >View Next Chapter{'>>'}</button>
           </div>
