...
export default function EnglishTranslationChapter31() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter32 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter32')
  }

 const loop31=[192,193]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 31. Description Of The Things Rasoolullah Drank</h2> </center>

    {
      loop31.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter31' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter31' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter32} >View Next Chapter{'>>'}</button>
           </div>
