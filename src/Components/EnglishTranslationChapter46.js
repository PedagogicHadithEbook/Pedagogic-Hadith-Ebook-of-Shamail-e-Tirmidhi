...
export default function EnglishTranslationChapter46() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter47 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter47')
  }

 const loop46=[306,307,308,309,310,311,312,313,314,315,316,317,318]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 46. The Humbleness Of Sayyidina Rasoolullah</h2></center>

    {
      loop46.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter46' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter46' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter47} >View Next Chapter{'>>'}</button>
           </div>
