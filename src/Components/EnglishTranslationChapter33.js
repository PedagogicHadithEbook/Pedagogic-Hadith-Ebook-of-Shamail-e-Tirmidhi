...
export default function EnglishTranslationChapter33() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter34 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter34')
  }

 const loop33=[204,205,206,207,208,209]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 33. Rasoolullah Using ‘Itr</h2> </center>

    {
      loop33.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter33' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter33' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter34} >View Next Chapter{'>>'}</button>
           </div>
