...
export default function EnglishTranslationChapter4() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter5 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter5')
  }

 const loop4=[30,31,32,33,34]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 4. The Combing Of The Hair Of Rasoolullah</h2> </center>

    {
      loop4.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter4' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter4' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter5} >View Next Chapter{'>>'}</button>
           </div>
