...
export default function EnglishTranslationChapter6() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter7 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter7')
  }

 const loop6=[43,44,45,46]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 6. Rasoolullah Using a Dye</h2> </center>

    {
      loop6.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter6' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter6' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter7} >View Next Chapter{'>>'}</button>
           </div>
