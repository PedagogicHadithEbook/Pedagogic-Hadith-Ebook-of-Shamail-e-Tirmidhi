...
export default function EnglishTranslationChapter17() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter18 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter18')
  }

 const loop17=[106,107,108,109,110]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 17. The Turban Of Rasoolullah</h2> </center>

    {
      loop17.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter17' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter17' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter18} >View Next Chapter{'>>'}</button>
           </div>
