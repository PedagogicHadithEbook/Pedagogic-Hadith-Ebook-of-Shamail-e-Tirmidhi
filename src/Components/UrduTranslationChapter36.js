...
export default function UrduTranslationChapter36() {
  
  const navigate = useNavigate();
  
  const UrduTranslationChapter37 = () => {
      navigate('/UrduTranslationChapters/UrduTranslationChapter37')
  }

 const loop36=[221,222,223,224,225,226]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <RWebShare
      ...
      {/* eslint-disable-next-line */}
      <h6><HashLink to={"/EnglishTranslationChapters/EnglishTranslationChapter36"+"/#Hadith"+(loop+1)} style={{ color:"midnightblue"}}>View English Translation</HashLink></h6>
       {/* eslint-disable-next-line */}
      <h6><HashLink to={"/WordToWordTranslationChapters/WordToWordTranslationChapter36"+"/#Hadith"+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {UrduTranslationChapter37} >View Next Chapter{'>>'}</button>
           </div>
   </>
  )
}
