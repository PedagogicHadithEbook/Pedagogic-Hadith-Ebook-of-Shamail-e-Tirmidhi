...
export default function EnglishTranslationChapter53() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter54 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter54')
  }

 const loop53=[360,361,362,363,364,365,366,367,368,369,370,371,372,373]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 53. The Death Of Sayyidina Rasoolullah</h2></center>

    {
      loop53.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter53' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
     {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter53' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter54} >View Next Chapter{'>>'}</button>
           </div>
