...
export default function EnglishTranslationChapter51() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter52 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter52')
  }

 const loop51=[345,346,347,348,349,350,351,352,353]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 51. The Living Of Sayyidina Rasoolullah</h2></center>

    {
      loop51.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter51' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
     {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter51' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter52} >View Next Chapter{'>>'}</button>
           </div>
