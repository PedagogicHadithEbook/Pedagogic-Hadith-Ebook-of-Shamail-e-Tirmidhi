...
export default function EnglishTranslationChapter48() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter49 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter49')
  }

 const loop48=[335,336]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 48. Modesty Of Sayyidina Rasoolullah</h2></center>

    {
      loop48.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter48' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter48' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter49} >View Next Chapter{'>>'}</button>
           </div>
