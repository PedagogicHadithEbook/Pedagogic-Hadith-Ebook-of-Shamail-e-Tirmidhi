...
export default function EnglishTranslationChapter50() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter51 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter51')
  }

 const loop50=[343,344]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 50. The Names Of Sayyidina Rasoolullah</h2></center>

    {
      loop50.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter50' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter50' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter51} >View Next Chapter{'>>'}</button>
           </div>
