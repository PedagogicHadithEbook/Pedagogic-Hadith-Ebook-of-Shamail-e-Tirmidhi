...
export default function EnglishTranslationChapter44() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter45 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter45')
  }

 const loop44=[299,300,301,302,303,304]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 44. The Weeping Of Sayyidina Rasoolullah</h2></center>

    {
      loop44.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter44' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>      
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter44' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
     </div>
           </div>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter45} >View Next Chapter{'>>'}</button>
           </div>
