...
export default function UrduTranslationChapter3() {
  
  const navigate = useNavigate();
  
  const UrduTranslationChapter4 = () => {
      navigate('/UrduTranslationChapters/UrduTranslationChapter4')
  }
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <RWebShare
      ...
      {/* eslint-disable-next-line */}
      <h6><HashLink to={"/EnglishTranslationChapters/EnglishTranslationChapter3"+"/#Hadith"+(loop+1)} style={{ color:"midnightblue"}}>View English Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {UrduTranslationChapter4} >View Next Chapter{'>>'}</button>
           </div>
   </>
  )
}
