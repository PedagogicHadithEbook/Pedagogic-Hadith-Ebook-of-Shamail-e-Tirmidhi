...
export default function EnglishTranslationChapter37() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter38 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter38')
  }

 const loop37=[227,228,229,230,231,232,233,234,235,236]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 37. Description Of The Saying Of Rasoolullah On Poetry</h2> </center>

    {
      loop37.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter37' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
     {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter37' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter38} >View Next Chapter{'>>'}</button>
           </div>
