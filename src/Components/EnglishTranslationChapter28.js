...
export default function EnglishTranslationChapter28() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter29 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter29')
  }

 const loop28=[177,178,179,180,181,182,183]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 28. The Words That Of Rasoolullah Said Before and After Eating</h2> </center>

    {
      loop28.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter28' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter28' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-To-Word Grammer</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter29} >View Next Chapter{'>>'}</button>
           </div>
