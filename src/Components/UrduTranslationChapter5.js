...
export default function UrduTranslationChapter5() {
  
  const navigate = useNavigate();
  
  const UrduTranslationChapter6 = () => {
      navigate('/UrduTranslationChapters/UrduTranslationChapter6')
  }
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <RWebShare
      ...
      {/* eslint-disable-next-line */}
      <h6><HashLink to={"/EnglishTranslationChapters/EnglishTranslationChapter5"+"/#Hadith"+(loop+1)} style={{ color:"midnightblue"}}>View English Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {UrduTranslationChapter6} >View Next Chapter{'>>'}</button>
           </div>
   </>
  )
}
