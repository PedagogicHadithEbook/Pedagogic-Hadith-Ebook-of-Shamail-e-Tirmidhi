...
export default function EnglishTranslationChapter54() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter55 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter55')
  }

 const loop54=[374,375,376,377,378,379,380,381,382]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 54. The Legacy Of Sayyidina Rasoolullah</h2></center>

    {
      loop54.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter54' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>      
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter54' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
     </div>
           </div>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter55} >View Next Chapter{'>>'}</button>
           </div>
