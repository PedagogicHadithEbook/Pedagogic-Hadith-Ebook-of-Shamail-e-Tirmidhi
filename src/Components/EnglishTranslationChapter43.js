...
export default function EnglishTranslationChapter43() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter44 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter44')
  }

 const loop43=[291,292,293,294,295,296,297,298]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 43. The Recital Of Sayyidina Rasoolullah</h2></center>

    {
      loop43.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter43' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter43' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter44} >View Next Chapter{'>>'}</button>
           </div>
