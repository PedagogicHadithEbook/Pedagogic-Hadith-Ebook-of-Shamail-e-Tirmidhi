...
export default function EnglishTranslationChapter20() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter21 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter21')
  }
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 20. The Qinaa Of Rasoolullah</h2> </center>

    {
      loop20.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter20' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter20' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter21} >View Next Chapter{'>>'}</button>
           </div>
