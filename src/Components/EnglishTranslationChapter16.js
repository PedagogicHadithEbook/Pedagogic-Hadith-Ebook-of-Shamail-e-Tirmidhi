...
export default function EnglishTranslationChapter16() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter17 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter17')
  }

 const loop16=[104,105]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 16. The Helmet Of Rasoolullah</h2> </center>

    {
      loop16.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter16' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter16' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter17} >View Next Chapter{'>>'}</button>
           </div>
