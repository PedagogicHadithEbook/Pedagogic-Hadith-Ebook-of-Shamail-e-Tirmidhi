...
export default function EnglishTranslationChapter7() {
  const navigate = useNavigate();
  
  const EnglishTranslationChapter8 = () => {
      navigate('/EnglishTranslationChapters/EnglishTranslationChapter8')
  }

 const loop7=[47,48,49,50]
...
const copyToClipboard = (loop) => {
  ...
  return (
    ...
    <center><h2>Chapter 7. Kuhl Of Rasoolullah</h2> </center>

    {
      loop7.map((loop,index) => (  
    ...
    {/* eslint-disable-next-line */}
      <h6><HashLink to={'/UrduTranslationChapters/UrduTranslationChapter7' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Urdu Translation</HashLink></h6>
      {/* eslint-disable-next-line */}
      <h6><HashLink to={'/WordToWordTranslationChapters/WordToWordTranslationChapter7' +'/#Hadith'+(loop+1)} style={{ color:"midnightblue"}}>View Word-By-Word Grammar</HashLink></h6>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
           <button class="NextButton" type="button" onClick = {EnglishTranslationChapter8} >View Next Chapter{'>>'}</button>
           </div>
